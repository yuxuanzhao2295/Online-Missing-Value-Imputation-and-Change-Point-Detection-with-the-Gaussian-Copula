...
def maskMNAR(X, seed, low_missing = 0.2, high_missing = 0.6, normal_missing = 0.4, 
            low_c = 0.87, high_c = 4.16):
    p_type = X.shape[1] // 3
    n, p = X.shape
    missing_prob = np.zeros((n,p))
    high_threshold = np.concatenate([high_c*np.ones(p_type), 0.5*np.ones(p_type), 4*np.ones(p_type)])
    low_threshold = np.concatenate([low_c*np.ones(p_type), 0.5*np.ones(p_type), 2*np.ones(p_type)])
    for i in range(n):
        x_row = X[i]
        missing_prob[i,x_row>=high_threshold ] = 0.2  
        missing_prob[i, (x_row>low_threshold) & (x_row<high_threshold)] = 0.4  
        missing_prob[i,x_row<=low_threshold ] = 0.6 
    np.random.seed(seed)
    _sample = np.random.uniform(size = (n,p))
    missing_indicator = _sample < missing_prob
    X_mask = X.copy()
    X_mask[missing_indicator] = np.nan
    return X_mask

def generate_data(seed=1, NUM_SAMPLES = 2000, write = False, path = None, p_type = 5):
    i = seed
    ...
    X_masked = maskMNAR(X, seed = seed)

    if write:
        name = f"{path}sim_online_rep{i}_mnar.csv"
...
def EM_evaluate(WINDOW_SIZE = 200, decay_coef=0.5, batch_c=5, NUM_SAMPLES=2000, BATCH_SIZE=40, 
    ...
    for i in range(START, NUM_STEPS+START):
        X_masked, X = generate_data(seed = i, NUM_SAMPLES=NUM_SAMPLES, p_type = p_type)
