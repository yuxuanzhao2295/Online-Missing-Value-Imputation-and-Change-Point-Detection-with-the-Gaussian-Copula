function [U,R,err_reg, residuals] = grouse(I,J,S,numr,numc,maxrank,step_size,maxCycles,Uinit, cons_stepsize)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k=1:numc,
    ...
    % EDIT: add tiny regularization to avoid Nan row in U_Omega
    
    %fprintf('Pass %d...\n',k);
    %[u,s,v] = svd(U_Omega, 'econ');
    %weights = v * diag(1./diag(s)) * u' * v_Omega;
    ...
    if t<pi/2, % drop big steps        
        ...
        update = step*weights';
        if any(isnan(update(:))) || any(isinf(update(:)))
            update = zeros(size(U));
        end
        %U = U + step*weights';
        U = U + update;
